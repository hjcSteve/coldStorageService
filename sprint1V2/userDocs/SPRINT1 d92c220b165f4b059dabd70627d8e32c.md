...
1. Nella directory sprint1Robot lanciare da terminale il comando `./gradlew run`  per avviare il contesto **CtxColdStorageService.** Questo espone **la porta 8055** per eventuali connessioni.
2. In intellij avviare il test MainCtxcoldstorageserviceKtTest.kt per inviare alla suddetta porta i messaggi di interesse.